...
\begin{solution}
    ...
    It can also be shown that $\lim\int_{G_n}f_n=\lim\int_{E^c}f_n$ using similar arguements as we did when working over $E$ and from the definition we see that $\int_{G_n}f_n\leq n^{-1}\int_{G_n}f_1\leq\alpha\int f_1$ and thus, $\lim\int_{E^c}f_n=0=\int_{E^c}f$ as $\alpha>0$ was arbitrary. 
