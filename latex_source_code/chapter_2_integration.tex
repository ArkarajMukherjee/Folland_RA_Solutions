...
\begin{solution}
	...
	By theorem $2.41c$, for each $E_j$ we can find a collection $\{R_{j,i}\}_{i=1}^{k_j}$ such that $m(E_j\Delta\cup_iR_{j,i})<\epsilon$ and each $R_{j,i}$ is a product of intervals, we will call these cubes here on out. 
...
\begin{solution}
	...
	\[
		\int |f|=\int_0^\infty\int_1^\infty se^{-sxy}|\sin(x)|dydx=\int_0^\infty e^{-sx}|\sin(x)/x|dx\leq\int_0^\infty e^{-sx}=s<\infty
	\]
...
\begin{solution}
	...
	\item Consider $f(x,y)=e^{-xy}\sin(x)$. 
		...
		\]
		\[
			=\lim_{b\to\infty}\int_0^\infty- \underbrace{\frac{e^{-by}\sin(b)+e^{-by}\cos(b)-1}{1+y^2}}_{g_b(y)}dy
		\]
		...
		\[
		    \int_0^b \frac{\sin x}{x}dx=\int_0^\infty \frac{1}{1+y^2}dy= \frac{\pi}{2}
		\]
    \end{enumerate}
\end{solution}
...
\begin{solution}
    \begin{enumerate}[label=\alph*.]
		\item By expanding we have that,
			...
			\[
				\frac{1}{\Gamma(\alpha)\Gamma(\beta)}\int_0^x\int_x^y(x-t)^{\alpha-1}(t-y)^{\beta-1}f(y)dtdy
			\]
			\[
				= \frac{1}{\Gamma(\alpha)\Gamma(\beta)}\int_0^xf(y)(x-y)^{\alpha+\beta-1}\int_x^y\qty( \frac{x-t}{x-y})^{\alpha-1}\qty( \frac{t-y}{x-y})^{\beta-1} \frac{dt}{x-y}dy
			\]
...
\begin{solution}
	...
	\[
		\int_{B(0,1/2)}\norm{x}^a|\log(\norm{x})|^b\,dm(x)=\sigma(S^{n-1})\int_0^{1/2} r^a\|log(r)|^bdr
	\]
...
\begin{enumerate}[label=(\alph*)]
	...
	\item By definition we see that $G_n(r,\omega)=rF_n(\omega)$, and as we had $|G_n(r,\omega)|=r$ we see that $F_n(\omega)=G_n(1,\omega)$ maps $\Omega'_n$ into $S^{n-1}$. 
		...
		\]
		\[
			=\int_{\Omega_n}\underbrace{\chi_{E_1}(G(r,\omega))}_{\chi_{(0,1]}(r)\cdot\chi_E(F(\omega))}|\det D_{(r,\omega)G}|\,dm((x,\omega))
		\]
		\[
		...
		\[
		\sigma(E)=\int_{F^{-1}|_{\Omega_n'}^{-1}(E)}\sin^{n-2}\phi_1\sin^{n-3}\phi_2\cdots\sin\phi_{n-2}\,\mathrm{d}m((\phi_1,\phi_2,\ldots,\phi_1))
		\]
