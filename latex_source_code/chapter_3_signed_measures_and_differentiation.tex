...
\]
where $c=m(B(0,1))$, as $|x-p|<r$ so we must have $H^*f\leq 2^nHf$.
\end{solution}
...
\begin{solution}
\begin{enumerate}[label=\alph*.]
\item Consider the borel measure $\nu(F):=\mu(E\cap F)$, then we have a lebesgue decomposition $\nu=0+\chi_Edm$ and Theorem $3.22.$ we have that $D_E(x)=\chi_E(x)$ for $\mu$-a.e. $x.$ from which the conclusion follows.
\item Consider $E=\bigcup_{k=1}^{\infty}(B(0,2^{-2k+1})\setminus B(0,2^{-2k}))$ and $x=0$. Let $c = m(B(0,1))$. Then we see that for $m\in\bN$ we have 
    ...
    Since $\frac{2^n}{2^n+1}\to 2$ and $ \frac{1}{2^n+1}\to 0$ for all $n \geq 1$, the limit depends on the sequence of radii chosen, hence $D_E(0)$ does not exist.
...
\end{problem}
\begin{solution}
    The first part is an exercise in calculus and the next follows from $F'$ being bounded ( $|F'|\leq3$ to be a bit more specific) and $T_G(1)-T_G(n^{-1/2})\gg n$ (this is not to be interpreted as $\infty-\infty$ but rather the total variation in the smaller interval $(n^{-1/2},1) )$
\end{solution}
...
\begin{solution}
    ...
    We have $\lambda_j \perp m$, and thus $\sum \lambda_j \perp m$ too by \hyperref[prob:3.9]{Problem 3.9} (notice that the proof is still valid if $\nu_j$ are complex measures as well.)
    ...
    If we now show that $F' = G'$ and $F_j' = G_j'$ a.e., then we would be done using the same lemma. Suppose $p$ and $q$ are such that $p = q$ a.e. and both are differentiable a.e. We want to show that $p' = q'$ a.e. We can easily find a set $A \subseteq [a,b]$ where $p = q$ and both are differentiable, such that $m([a,b] \setminus A) = 0$. 
