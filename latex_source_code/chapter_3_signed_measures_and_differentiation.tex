...
\begin{enumerate}[label=\alph*.]
    \item Following steps similar to those the proof of theorem 3.36, taking $\Omega=\{(x,y):a\leq x\leq y\leq b\}$ we can see that,
        ...
        \[
            \int_{[a,b]}(F(x)+F(x-))dF(x)+\int_{[a,b]}(G(x)+G(x-))dF(x)=2(F(b)G(b)-F(a-)G(a-))
        \]
...
$G$ is also continuous by weirstrass' M-test being the limit of an uniformly converging sequence of continuous functions. 
...
We see that $G$ is constant on intervals outside the cantor set and thus has derivative $0$ here and as the cantor set has lebesgue measure $0$ we can show that $G'=0$ a.e. by standard procedures. 
