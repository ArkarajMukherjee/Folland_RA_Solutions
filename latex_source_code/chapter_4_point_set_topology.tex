...
\begin{solution}
    ...
    Thus, all the sets in the generator either have both $x,y$ or none of these and by proposition 4.4 the open sets too have this property and are unable to seperate these distinct points which makes $\mathcal T$ not hausdorff so we are done. 
\end{solution}
...
Thus, $\mathcal T$ is contained in this weak topology (the inclusion in the other direction holds from the definition of weak topologies.)  
\end{solution}
...
\begin{solution}
\begin{enumerate}[label=\alph*.]
    \item Clearly $\emptyset,\tilde{X}\in\mathcal T$, the rest follows from $\pi^{-1}$ commuting with unions and intersections. 
...
\end{problem}
\begin{solution}
If $\mathcal T'$ is coarser then it is clearly compact as all open covers in it are also open covers in $\mathcal T$ and if its finer then it is hausdorff as all neighbourhoods in $\mathcal T$ are also neighbourhoods in $\mathcal T'$. 
Now, if $\mathcal T'$ is finer then the identity map from $\mathcal T'$ to $\mathcal T$ is continuous. 
If $\mathcal T'$ is compact then its closed sets are also compact and thus get mapped to compact sets under the identity map, which are closed in $\mathcal T$ as it is hausdorff hence implying $\mathcal T' \subseteq \mathcal T$ so $\mathcal T'$ can not be strictly finer. 
Now, if $\mathcal T'$ was strictly coarser then by what we just proved we see that this can not be hausdorff.
\end{solution}
...
\begin{solution}
    ...
    Further, as this has no accumulation points, there exists open neighbourhoods of $x_m$ that omits all other points in $A$ and thus, in the relative topology of $A$ $ \qty{x_m}\cap A= \qty{x_m}$ is open and thus $A$ inherits a discrete topology as its subset topology (relative topology whatever), this means that $C(A)=\bC^A$. 
